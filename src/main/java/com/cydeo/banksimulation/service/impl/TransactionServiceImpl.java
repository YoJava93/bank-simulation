...
import com.cydeo.banksimulation.exception.NotSufficientBalanceException;
import com.cydeo.banksimulation.exception.UnerConstructionException;
...
public class TransactionServiceImpl implements TransactionService {
    ...
    public Transaction makeTransfer(BigDecimal amount, Date creationDate, Account sender, Account receiver, String message) {
        if (underConstruction) {
            ...
            return transactionRepository.save(Transaction.builder().amount(amount)
                    .creationDate(creationDate)
                    .sender(sender.getId())
                    .receiver(receiver.getId())
                    .message(message).build());
        }else {
            throw new UnerConstructionException("Make transfer is not possible for now. Please try again later");
        }
    ...
    private void executeBalanceAndUpdateIfRequired(BigDecimal amount, Account sender, Account receiver) {
        if (checkSenderBalance(sender, amount)){
            ...
            throw new NotSufficientBalanceException("Balance is not enough for this transaction");
        }
    }
